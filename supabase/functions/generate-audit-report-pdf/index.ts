// ============================================================================
// generate-audit-report-pdf — gera PDF do relatório consolidado e devolve
// uma URL assinada (signed URL) curta para compartilhamento com marcas.
// ----------------------------------------------------------------------------
// Strategy: monta o PDF puro em bytes (sem libs externas pesadas — escrevemos
// o spec PDF 1.4 manualmente) e faz upload para o bucket privado
// `audit-reports`. Em seguida cria uma signed URL com TTL configurável.
// ============================================================================
...
// Sanitize to WinAnsi-printable; replace anything weird with '?'
function asciiSafe(s: string): string {
  return (s ?? "")
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "") // strip accents (Helvetica WinAnsi safe-ish)
    .replace(/[^\x20-\x7E]/g, "?");
}

// HSL→RGB (returns 0..1 floats for PDF)
...
// Generate N visually distinct colors using golden ratio hue rotation
...
function buildPdf(report: ReportPayload, signedExpiresAt: Date): Uint8Array {
  // Aggregate provider seconds
  ...
  // We accumulate content stream commands
  const commands: string[] = [];

  // helpers (text in BT/ET blocks; we open/close per chunk for simplicity)
  ...
  // pie slice approximated with triangle fan from center
  ...
  text("Relatorio Consolidado de Auditoria de VOD", MARGIN, PAGE_H - 48, 11, false);
  ...
  setFill(0.1, 0.1, 0.1);
  text("RESUMO EXECUTIVO", MARGIN + 10, cursorY - 14, 9, true);
  ...
  const detectedPct = report.vod_duration_seconds > 0
    ...
    : 0;
  const cards = [
    { label: "DURACAO TOTAL", value: fmtDuration(report.vod_duration_seconds) },
    { label: "TEMPO DE CASSINO", value: fmtDuration(report.total_casino_seconds) },
    { label: "% DO STREAM", value: `${detectedPct.toFixed(1)}%` },
    { label: "PROVEDORAS", value: String(providers.length) },
  ];
  ...
  if (report.games.length > 0) {
    ...
    text("FRAMES", MARGIN + 380, cursorY - 10, 8, true);
  ...
  setFill(0.4, 0.4, 0.45);
  text("Fonte: varredura visual interna (raw_evidences) - Auditoria autonoma Starklytic", MARGIN, MARGIN + 6, 7);
  text(`audit_id: ${report.audit_id}`, MARGIN, MARGIN - 6, 6);
  ...
  // patch pageObj parent reference
...
Deno.serve(async (req) => {
  ...
  try {
    // Auth: require an authenticated user (any role) — we use service role for storage ops
    ...
    const ttlSeconds = Math.min(Math.max(Number(body?.ttl_seconds) || DEFAULT_TTL_SECONDS, 60), 60 * 60 * 24 * 30);
    if (!auditId) return jsonResponse({ error: "audit_id is required" }, 400);

    // Fetch consolidated report by invoking the existing vod-watcher-agent
    const reportResp = await fetch(`${SUPABASE_URL}/functions/v1/vod-watcher-agent`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ action: "report", audit_id: auditId }),
    });
    if (!reportResp.ok) {
      const t = await reportResp.text();
      return jsonResponse({ error: `Falha ao buscar relatorio: ${t.slice(0, 200)}` }, 500);
    }
    const report = (await reportResp.json()) as ReportPayload;
    if ((report as any)?.error) return jsonResponse({ error: (report as any).error }, 500);

    const expiresAt = new Date(Date.now() + ttlSeconds * 1000);
    const pdfBytes = buildPdf(report, expiresAt);

    const admin = createClient(SUPABASE_URL, SERVICE_KEY);
    const path = `${report.streamer_login}/${report.vod_id}-${Date.now()}.pdf`;
    ...
    const { data: signed, error: signErr } = await admin.storage
      .from(BUCKET)
      .createSignedUrl(path, ttlSeconds, { download: `relatorio-${report.streamer_login}-${report.vod_id}.pdf` });
    ...
    return jsonResponse({
      ...
      filename: `relatorio-${report.streamer_login}-${report.vod_id}.pdf`,
